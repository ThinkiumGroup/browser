<template>
  <div class="mobile-addressDetails-container container-background-clor">
    ...
    <div class="mobile-addressDetails-body">
      ...
      >
        <CardNav :isMore="data.length >= 5" :title="$t('title.external_transaction')" @loadMore="loadMoreTrading" />
      ...
      <div
        class="mobile-addressDetails-body-list mobile-block"
      >
        <CardNav :isMore="blockData.length >= 5" :title="$t('title.Inside_transaction')" @loadMore="loadMoreTrading" />
          <TransactionBlock
            type="internal"
...
export default {
  ...
  created() {
    ...
    getTaskAddressDetails({ address: this.transformAddress(this.address, '0x') }).then(res => {
  ...
  methods: {
    getTradingData() {
      ...
      const data = {
        ...
        address: this.transformAddress(this.address, '0x'),
    ...
    loadMoreTrading() {
      this.$router.push(`/allTransaction?address=${this.transformAddress(this.address, 'TH')}`)
    }
