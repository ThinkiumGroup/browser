<template>
  <div class="page-wrap">
    ...
    <div class="content-wrap">
      <div class="content-left common-block" v-if="currencyPc">
        <div class="header">
          ...
          <span>{{$t('title.hold_details')}}</span>
        </div>
      ...
      <div class="content-right common-block">
        ...
        <div class="tab-wrap">
          <div class="tab-item" :class="{'active': activeName == 'externalTransaction'}" @click="selectClick('externalTransaction')">{{$t('title.external_transaction')}}</div>
          <div class="tab-item" :class="{'active': activeName == 'insideTransaction'}" @click="selectClick('insideTransaction')">{{$t('title.Inside_transaction')}}</div>
        </div>
    ...
    <el-tabs v-model="activeName" @tab-click="handleClick" style="margin-top: 30px">
      <el-tab-pane :label="$t('title.external_transaction')" name="externalTransaction">
        <tradeInfoList
          v-loading="loading"
          :currencyPc="currencyPc"
          :isShow="true"
      ...
      <el-tab-pane :label="$t('title.Inside_transaction')" name="insideTransaction">
        <el-table
          v-loading="loading"
          :data="orderType != 1 ? tableData.slice( (currentPage-1)*pagesize, currentPage*pagesize) : tableData.slice( (myCurrentPage-2 < 0 ? 0 : ((myCurrentPage-2)*pagesize + beginRows)), (myCurrentPage-1)*pagesize + beginRows).reverse()"
