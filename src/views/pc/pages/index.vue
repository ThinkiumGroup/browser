    ...
    methods: {
      ...
      goSearchData() {
        if (this.search_transaction == '') {
          ...
          if(/^0x/g.test(this.search_transaction) == false || this.search_transaction.length === 42){
             let address = /^0x/g.test(this.search_transaction) ? this.to32Decimal(this.search_transaction) : this.search_transaction;
              this.$router.push({
                path: 'addressDetail',
                query: {
                  address,
