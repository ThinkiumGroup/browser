<template>
  <!-- 这个是主链和任务链公共的页面  -->
  <div class="page-wrap">
    <div class="main-title">{{$t(chainName)}}{{$t('title.详情')}}</div>
    <div class="common-header header-wrap">
      <div>
        <div class="dot"></div>
        <span class="label">{{$t('title.当前块高')}}</span>
        <span class="left-value">{{all_block_data[0] && all_block_data[0].blockHeight}}</span>
      </div>
      <div>
        <div class="dot"></div>
        <span class="label">{{$t('title.共识委员会参选轮次')}}</span>
        <span class="left-value">{{runRound}}</span>
        <span class="unit">{{$t('title.届')}}</span>
      </div>
      <div>
        <div class="dot"></div>
        <span class="label">{{$t('title.全网交易')}}</span>
    ...
    <div class="content-wrap">
        ...
        <div class="content-right-bottom common-block">
          <div class="header">
            <div class="header-left">
              ...
              <!-- <span>（共{{runUserInfo.length}}个共识节点或矿池）</span> -->
            </div>
      ...
      <div class="content-left common-block">
        ...
        <div class="total-wrap">
          ...
          <div class="right-wrap">{{$t('title.当前共')}}<span> {{tradeTotal}} </span>{{$t('title.条交易')}}</div>
        </div>
